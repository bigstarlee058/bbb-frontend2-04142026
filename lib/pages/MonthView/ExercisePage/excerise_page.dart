...
import '../MonthViewPage/sections/choose_equipment_popup.dart';
...
class _ExercisePageState extends State<ExercisePage> with TickerProviderStateMixin {
  ...
  final ValueNotifier<Duration> videoProgressValue1 = ValueNotifier(Duration.zero);
  ...
  Future<void> fromNotification() async {
    ...
    if (monthProvider!.isPumpDay) {
      ...
      if (monthProvider!.allDayHistoryModel
          .any((element) => element.dataId == dataId && element.type!.contains("Pump Day"))) {
  ...
  Future<void> fetchExercise(
      {String? exerciseId, int? exerciseIndex, bool? isPumpDay, bool? isCircuit, String? circuitIndex}) async {
  ...
  fetchExtraSetLocalData(String dataId) async {
    final data = await DatabaseHelper()
        .getDataFromTable(tableName: DatabaseHelper.extraSetHistory, where: 'dataId', id: "EXTRA-ADDED$dataId");
    if (data.isNotEmpty) {
                        ...
                        children: [
                          ...
                          GestureDetector(
                            ...
                            child: Container(
                              height: ScreenUtil.verticalScale(4),
                              width: ScreenUtil.verticalScale(4),
                              padding: EdgeInsets.all(ScreenUtil.verticalScale(0.55)),
                              decoration: BoxDecoration(color: Color(0XFFd18a9b), shape: BoxShape.circle),
                              child: Center(
                                child: SvgPicture.asset(
                    ...
                    if (monthProvider!.isCurrentMonth == "Future") ...[
                          ...
                          ? Padding(
                              ...
                              child: TextButton(
                                onPressed: () async {
                                  ...
                                  final data =
                                      monthProvider?.selectedExercise!.extra!.where((element) => element.type == 3);
                                  if (data!.isNotEmpty) {
                      ...
                      if (isCurrentDayCompleted || isCurrentDaySkipped) ...[
                                          ...
                                          WidgetsBinding.instance.addPostFrameCallback(
                                            (timeStamp) async {
                                              ...
                                              if (monthProvider.isPumpDay && monthProvider.isCircuit) {
                                                ...
                                                var circuitExercises = monthProvider
                                                    .pumpDayModel!.circuits![circuitIndex].circuitExercises!;
                                                ...
                                                exerciseList.addAll(monthProvider.dayDataModel!.exercises!.where(
                                                    (element) =>
                                                ...
                                                if (exerciseList.length != count &&
                                                  ...
                                                  for (int i = skipIndex; i < exerciseList.length; i++) {
                                                    ...
                                                    if (val == false) {
                                                      ...
                                                      bool isLast = i ==
                                                          exerciseList.indexWhere((element) =>
                        ...
                        Consumer<MonthProvider>(
                          builder: (context, monthProvider, child) {
                            return ButtonWidget(
                              text: monthProvider.exerciseHistoryDetails?.status == Status.skipped
                                  ? "Unskip?"
                                  : "Skip the exercise",
                              ...
                              onPress: () async {
                                WidgetsBinding.instance.addPostFrameCallback(
                                  (timeStamp) async {
                                    ...
                                    await _saveExerciseData(
                                      status: monthProvider.exerciseHistoryDetails?.status == Status.skipped
                                          ? ""
                                          : "Skipped",
                                      id: monthProvider.isPumpDay && monthProvider.isCircuit
                                          ? "${monthProvider.exerciseDetailModel!.sId.toString()}-${monthProvider.circuitIndex}"
  ...
  Future<void> _saveExerciseData({required String status, required String id, required String type}) async {
    ...
    if (monthProvider!.isPumpDay && monthProvider!.isCircuit) {
      ...
      final data2 = {
        ...
        "lastExerciseCount": 1,
        "lastRound":
            monthProvider?.pumpDayModel!.circuits?[monthProvider!.circuitsIndex].circuitExercises?.length == 1 ? 2 : 1,
        "exerciseCountList":
            monthProvider?.pumpDayModel!.circuits?[monthProvider!.circuitsIndex].circuitExercises?.length == 1
                ? ""
                : monthProvider!.exerciseDetailModel!.sId.toString(),
      };

      if (monthProvider!.circuitModel.isNotEmpty) {
        CircuitModel? matchingElement = monthProvider?.circuitModel
            .firstWhere((element) => element.dataId == dataId1, orElse: () => CircuitModel());
        ...
        if (matchingElement?.id != null) {
          if (!matchingElement!.exerciseCountList!.contains(monthProvider!.exerciseDetailModel!.sId.toString())) {
            ...
            if (monthProvider?.pumpDayModel!.circuits?[monthProvider!.circuitsIndex].circuitExercises?.length == 1) {
              ...
              data3 = {
                "dataId": dataId1,
                "lastExerciseCount": matchingElement.exerciseCountList.toString().replaceAll("-", "").isEmpty
                    ? 1
                    : (matchingElement.lastExerciseCount! + 1),
