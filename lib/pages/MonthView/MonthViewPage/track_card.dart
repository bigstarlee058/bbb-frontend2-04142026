...
class _WeeklyTrackCardState extends State<WeeklyTrackCard> {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      if (monthProvider!.isCurrentMonth == "Current") {
        if ((mainIndex! + 1) == monthProvider?.week ? true : false) {
          monthProvider!.updateWeekExpandedHeight(84);
        }
        ...
        children: [
          ...
          ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              DayDataModel dayData = weekDataModel!.dayList![index].toString().contains("Workout")
              ...
              if (exerciseDetails != null && monthProvider.allRemovedExercise.isNotEmpty) {
                String split =
                    monthProvider.monthDataModel?.weeks?[mainIndex!].idList?.first.toString().split(" ")[1] ?? "";
              ...
              int nextWorkOutIndex = weekDataModel!.dayList![index].toString().contains("Workout")
                  ? int.parse(
                          weekDataModel!.dayList![index].toString().replaceAll("Day ", "").replaceAll(" Workout", "")) -
                      1
                  : 0;
              return weekDataModel!.dayList![index].toString().contains("Workout")
                  ? workoutday(
                      monthProvider, isRestDay, dataId, index, dayData, context, nextWorkOutIndex, exerciseCount)
  ...
  Widget workoutday(MonthProvider monthProvider, bool isRestDay, String dataId, int index, DayDataModel dayData,
      BuildContext context, int nextWorkOutIndex, int? exerciseCount) {
    return Padding(
              ...
              decoration: BoxDecoration(
                color: monthProvider.weekStatuses[mainIndex!] == WeekType.futureWeek
                    ...
                    : (monthProvider.weekStatuses[mainIndex!] == WeekType.pastWeek &&
                                monthProvider.allSplitDayHistoryModel.isEmpty) ||
                            monthProvider.allSplitDayHistoryModel
                                .any((e) => e.status == Status.skipped && e.dataId == dataId)
                        ? AppColors.skipDayColor
                        : monthProvider.allSplitDayHistoryModel
                                .any((e) => e.status == Status.completed && e.dataId == dataId)
                            ? Colors.green
  ...
  Widget selection(
      MonthProvider monthProvider, int index, DayDataModel dayData, BuildContext context, String dayId, String dataId) {
    return Row(
      children: [
        Expanded(
            ...
            onTap: () async {
              ...
              if (dataList.isNotEmpty) {
                int index1 = monthProvider.pumpDays.indexWhere((el1) => dataList
                    .any((e1) => (e1.dayId == dayId && e1.type.toString().replaceAll("Pump Day - ", "") == el1.id)));
                if (index1 != -1) {
                ...
                await Navigator.pushNamed(context, '/today').then(
                  (value) {
                    WidgetsBinding.instance
                        .addPostFrameCallback((timeStamp) async => await monthProvider.checkForPumpDay());
                  },
                );
              } else {
                if (!context.mounted) return;
  ...
  Widget pumpDayRestday(MonthProvider monthProvider, String dataId, int index, bool isRestDay, DayDataModel dayData) {
    return Builder(builder: (context) {
      DayHistoryModel? matchingElement = monthProvider.allSplitDayHistoryModel.firstWhere(
                  ...
                  child: SizedBox(
                    ...
                    child: InkWell(
                      ...
                      child: Row(
                        ...
                        children: [
                          Builder(builder: (context) {
                            int i = int.parse(weekDataModel!.dayList![index].toString().split(" ").toList().last) - 1;
  ...
  bool isRestPumpOption(bool isRestDay, MonthProvider monthProvider, String dataId) {
    return ((isRestDay &&
                monthProvider.weekStatuses[mainIndex!] == WeekType.currentWeek &&
                monthProvider.isPumpDayAvailable) &&
            (monthProvider.allSplitDayHistoryModel.any((element) => (element.status != Status.skipped &&
        ...
        ((isRestDay && monthProvider.weekStatuses[mainIndex!] == WeekType.currentWeek) &&
            (monthProvider.allSplitDayHistoryModel
                .any((element) => ((element.status == Status.started) && dataId == element.dataId))));
  }

  Future<void> continueWorkoutOnTap(bool isRestDay, String dataId, int index, DayDataModel dayData,
      BuildContext context, int weekIndex, String dayId) async {
    DayHistoryModel? matchingElement = monthProvider!.allSplitDayHistoryModel.firstWhere(
        (element) => element.dataId == dataId && element.type!.contains("Pump Day"),
        orElse: () => DayHistoryModel());

    bool isRestDayForPastWeek = monthProvider!.weekStatuses[mainIndex!] == WeekType.pastWeek &&
    ...
    if (isPumpDay) {
      ...
      if (dataList!.isNotEmpty) {
        int index1 = monthProvider!.pumpDays.indexWhere((el1) =>
            dataList.any((e1) => (e1.dayId == dayId && e1.type.toString().replaceAll("Pump Day - ", "") == el1.id)));
        if (index1 != -1) {
          monthProvider?.updatePumpDayData(monthProvider!.pumpDays[index1]);
        } else {
  ...
  Future<void> showWorkout(bool isRestDay, String dataId, int index, DayDataModel dayData, BuildContext context,
      int weekIndex, String dayId) async {
    DayHistoryModel? matchingElement = monthProvider!.allSplitDayHistoryModel.firstWhere(
        (element) => element.dataId == dataId && element.type!.contains("Pump Day"),
        orElse: () => DayHistoryModel());
