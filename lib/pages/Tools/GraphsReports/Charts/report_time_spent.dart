...
class ReportTimeSpentGraph extends StatefulWidget {
  const ReportTimeSpentGraph({super.key});

  @override
  State<ReportTimeSpentGraph> createState() => _ReportTimeSpentGraphState();
}

class _ReportTimeSpentGraphState extends State<ReportTimeSpentGraph> {
  BarChartGroupData generateBarGroup(
    ...
    return BarChartGroupData(
      ...
      barRods: [
        BarChartRodData(
          toY: double.parse((value / 3600).toStringAsFixed(2)),
          color: color,
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) => monthProvider.changeWeekTimeSpent("Week ${monthProvider.currentWeek}"));
    super.initState();
  }
        ...
        return AspectRatio(
          ...
          child: BarChart(
            BarChartData(
              ...
              titlesData: FlTitlesData(
                ...
                leftTitles: AxisTitles(
                  sideTitles: SideTitles(
                    showTitles: true,
                    interval: monthProvider.reportMaximumValueOfTotalTime > 8 ? monthProvider.reportMaximumValueOfTotalTime / 10 : 1,
              ...
              barGroups: monthProvider.reportTimeSpentGraphHistory.asMap().entries.map((e) {
                final index = e.key;
              ...
              maxY: monthProvider.reportMaximumValueOfTotalTime > 8 ? monthProvider.reportMaximumValueOfTotalTime : 8,
  ...
  Widget getTitles(double value, TitleMeta meta, MonthProvider monthProvider) {
    List titles = monthProvider.reportTimeSpentGraphHistory.asMap().entries.map((e) {
      return e.value['day'];
    }).toList();

    final isd = int.parse(monthProvider.reportTimeSpent.toString().replaceAll("Week ", ""));
